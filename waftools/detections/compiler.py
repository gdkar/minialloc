...
def __add_generic_flags__(ctx):
    ctx.env.CFLAGS += ["-D_ISOC11_SOURCE", "-D_GNU_SOURCE",
...
def __add_gcc_flags__(ctx):
    ctx.env.CFLAGS += ['-std=gnu11', "-Wall", "-Wundef", "-Wmissing-prototypes", "-Wshadow",
                       "-Wno-switch", "-Wno-parentheses", "-Wpointer-arith",
                       "-Wredundant-decls", "-Wno-pointer-sign"]

def __add_clang_flags__(ctx):
    ctx.env.CFLAGS += ['-std=c11',"-Wno-logical-op-parentheses", "-fcolor-diagnostics",
